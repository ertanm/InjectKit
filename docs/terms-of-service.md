# PromptVault Terms of Service

**Last updated:** March 21, 2026

## 1. Acceptance of Terms

By using PromptVault, you agree to these Terms of Service. If you do not agree, do not use the service.

## 2. Service Description

PromptVault is a browser extension and cloud service for saving, organizing, and injecting AI prompts into chat interfaces.

## 3. User Accounts

- You must provide accurate information when creating an account
- You are responsible for maintaining the security of your account
- You must not share your account credentials

## 4. Free and Pro Plans

- **Free plan**: Up to 50 prompts, basic features
- **Pro plan**: Unlimited prompts, version history, usage analytics, and import/export features available through the API; the extension may not expose every Pro capability in the UI yet
- Pricing: EUR 8.99/month or EUR 89/year (subject to change; see checkout)

## 5. Acceptable Use

You agree not to:
- Use the service for any illegal purpose
- Store prompts containing malware, phishing, or harmful content
- Attempt to access other users' data
- Reverse-engineer or exploit the service

## 6. Content Ownership

- You retain ownership of all prompts and content you create
- You grant us a limited license to store, display, and transmit your content to provide the service
- We do not claim ownership of your content

## 7. Service Availability

- We strive for high uptime but do not guarantee it
- We may perform maintenance with reasonable notice
- We reserve the right to modify or discontinue features

## 8. Cancellation and Refunds

- You may cancel your Pro subscription at any time
- Cancellation takes effect at the end of the current billing period
- Refunds are handled on a case-by-case basis within 14 days of purchase

## 9. Limitation of Liability

PromptVault is provided "as is" without warranties. We are not liable for any indirect, incidental, or consequential damages.

## 10. Governing Law

These terms are governed by the laws of the European Union and the applicable member state.

## 11. Contact

For questions about these terms: legal@promptvault.dev
